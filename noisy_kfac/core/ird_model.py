...
from ..ops import adam_optimizer as opt
...
class IRDModel(BaseModel):
    ...
    def trainable_variables(self):
        raise NotImplementedError("optimizer only needs param_dict")
    ...
    def init_optim(self):
        self.optim = opt.AdamOptimizer(
            ...
            damping_int=self.config.kl/self.n_data/self.config.eta,
            damping_ext=self.config.damping,
            fisher_momentum=self.config.cov_ema_decay,
            momentum=self.config.mean_decay,
            )
